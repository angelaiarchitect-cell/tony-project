...
export const TOOLS = [
  ...
  // ═══ GOOGLE DRIVE ═══
  {
    ...
    description: "List files from Google Drive. Use when the user wants to see their files, browse Drive, or check what's in a folder. Can filter by file type (sheets, docs, folders, pdf, slides).",
    input_schema: {
      type: "object",
      properties: {
        folder_id: { type: "string", description: "Folder ID to list contents of. Omit for root/recent files." },
        file_type: { type: "string", enum: ["sheets", "docs", "folders", "slides", "pdf"], description: "Filter by file type. Omit for all types." },
        max_results: { type: "number", description: "Number of files to return. Default 20." },
      },
    },
  },
  {
    name: "google_drive_search",
    description: "Search Google Drive for files by name. Use when the user asks to find a specific file, spreadsheet, or document by name. Use this FIRST when the user says 'open [name]' to find the file ID.",
    input_schema: {
      type: "object",
      properties: {
        query: { type: "string", description: "Search term (file name or partial name, e.g. 'AACM', 'budget', 'Site 11250')" },
        file_type: { type: "string", enum: ["sheets", "docs", "folders", "slides", "pdf"], description: "Filter by file type" },
  ...
  {
    name: "google_drive_open_spreadsheet",
    description: "Search for and open a Google Sheet by name — returns the full spreadsheet data (headers, rows, sheet names). Use when the user says 'open [name] in sheets', 'show me the [name] spreadsheet', or 'pull up [name]'. This is the ONE-STOP tool for finding and reading a Google Sheet by name.",
    input_schema: {
      type: "object",
      properties: {
        name: { type: "string", description: "Name or partial name of the spreadsheet (e.g. 'AACM', 'budget tracker', 'Site 11250')" },
      },
      required: ["name"],
    },
  },
...

  {
    ...
    description: "Download and read the contents of an email attachment. Works with CSV, TSV, JSON, TXT, HTML, and image files — parses them and returns the data directly. For binary files (PDF, XLSX, DOCX), returns metadata. Use after reading an email that has attachments.",
    input_schema: {
      type: "object",
      properties: {
        message_id: { type: "string", description: "The Gmail message ID that contains the attachment" },
        attachment_id: { type: "string", description: "The attachment ID from the email's attachment list" },
        filename: { type: "string", description: "The filename of the attachment (used for type detection)" },
      },
      required: ["message_id", "attachment_id", "filename"],
    },
  },
  {
    name: "gmail_analyze_attachments",
    description: "Analyze ALL attachments on an email at once. Automatically downloads, parses, and returns the data from every attachment. Best for emails with multiple files — like reports, invoices, or data exports. Use when the user says 'look at the attachments' or 'what's in that file'.",
    input_schema: {
      type: "object",
      properties: {
        message_id: { type: "string", description: "The Gmail message ID to analyze attachments from" },
      },
      required: ["message_id"],
    },
  },
