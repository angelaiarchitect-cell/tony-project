...
export async function getAttachment({ message_id, attachment_id, filename }) {
  ...
  // For binary formats we can't parse serverside (PDF, XLSX, DOCX)
  // Return metadata and suggest user download from Gmail
  return {
    success: true,
    filename,
    mimeType,
    ...
    data: {
      type: "binary",
      message: `File "${filename}" (${(buffer.length / 1024).toFixed(1)} KB) is a binary ${mimeType} file. I can see it's attached but can't parse this format directly. You can download it from Gmail to view its contents.`,
    },
  };
}
