...
async function getAccessToken() {
  console.log(`[Drive] Token refresh - client_id exists: ${!!process.env.GOOGLE_CLIENT_ID}, secret exists: ${!!process.env.GOOGLE_CLIENT_SECRET}, refresh exists: ${!!process.env.GOOGLE_REFRESH_TOKEN}`);
  ...
  if (data.error) {
    console.error(`[Drive] Auth error: ${data.error} - ${data.error_description}`);
    throw new Error(`Google auth error: ${data.error_description || data.error}`);
  }
  console.log(`[Drive] Got access token, length: ${data.access_token?.length}`);
